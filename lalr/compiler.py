...
class LALRParser:
                    ...
                    if len(l) == 2:
                        if l[0][0] == 'shift' or l[1][0] == 'shift':  # shift-reduce, can be solved
                            ...
                            if len(self.invertedRuleDict[r[1]][1]) > 1:
                                opReduce = self.invertedRuleDict[r[1]][1][
                                    -2]  # get r[1] (reduce index), then get second element (right part of the production), then get the operator
                                opShift = j
    ...
    def hashLR0item(self,
                    node):  # create hashable element with all rules of a LR(0) item (ignoring lookaheads), useful to merge nodes
        s = set()
        for rule in node:
    ...
    def closure(self, rules, remainingSymbols, symbol, lookahead):
        for right in self.grammar.productions[symbol]:
            ...
            if s in rules and len(
                            lookahead - rules[s]) == 0:  # rule exists, and lookahead is included in the existing one
                continue
